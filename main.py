...
cluster = MongoClient(
    "mongodb+srv://soren:cdD2_qWUYRk-d4G@orion.iztml.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
)
base = cluster["OrionDB"]

fc_cur = base[
    "fc"]  #Formation = [_id, Guild, Channel, Past_Number, Last_Number, Author]
    ...
    if member.guild.id in guilds:
        ...
        if banner_guild != None:
            ...
            def transparent_circle(img):
                ...
                circ = cv.circle(circ, (width // 2, height // 2),
                                 min(height, width) // 2, (1),
                                 -1,
                                 lineType=cv.LINE_AA)
            ...
            def copyTransparent(mainImage, transparentImage, x, y):
                ...
                for c in range(0, 3):
                    mainImage[y1:y2, x1:x2,
                              c] = (alpha_s * transparentImage[:, :, c] +
                                    alpha_l * mainImage[y1:y2, x1:x2, c])

                return mainImage

            xc = center_coord[0] - radius
            yc = center_coord[1] - radius
            transparent_copied = copyTransparent(bg_resize.copy(), pfp_png, xc,
                                                 yc)

            # show(transparent_copied)

            # circle_border_color = (0,0,255) # in BGR
            cv.circle(transparent_copied,
                      center_coord,
                      radius,
                      colors[circle_color],
                      5,
            ...
            def customTextCenter(img, string, fontPath, textSize, fontColor,
                                 h):
            ...
            after_hello = customTextCenter(transparent_copied, hello_string,
                                           FONT_PATH, 90,
                                           colors[welcome_color],
                                           2 * (height // 3) - 5)
            # show(after_hello)

            after_welcome = customTextCenter(after_hello, welcome_string,
                                             FONT_PATH, 50, colors[name_color],
                                             2 * (height // 3) + 80)
            # show(after_welcome)
...
async def invite(ctx):
    ...
    embed.add_field(
        ...
        value=
        "[Click here](https://discord.com/api/oauth2/authorize?client_id=777095257262522399&permissions=8&scope=bot) to invite me in your server.\n឵឵",
        inline=False)
    embed.add_field(
        name="SERVER INVITATION LINK",
        value=
        ":small_blue_diamond: [Click here](https://discord.gg/JJtUtgMjBv) to join my creator's server.",
        inline=False)
    embed.set_footer(icon_url=ctx.author.avatar_url,
                     text=f"Thank you very much {ctx.author.name}")
...
@client.command()
async def check_permission(ctx, channel1: discord.TextChannel,
                           user: discord.Member):
...
async def youtube(ctx, *, search):
    ...
    def react_check(reaction, user):  #reaction check function
        emojis = ["🚫", "➡️", "⬅️"]
        return user.id == ctx.author.id and reaction.message.id == link_msg.id and str(
    ...
    while True:  # looping between pages
        ...
        try:  # to handle timeout error
            user_react, user = await client.wait_for("reaction_add",
                                                     check=react_check,
                                                     timeout=60)

            if user_react.emoji == "➡️" and page != pages - 1:
                page += 1
                await link_msg.edit(content='https://www.youtube.com' +
                                    search_results[page])
...
async def on_message(message):
    ...
    if exact_txt == "i am sad":
        await message.channel.send(
            f"{message.author.mention} why? :pleading_face:")
    ...
    if message.author.id == 693375549686415381:
        ...
        if exact_txt_splitted[0] == "send_ch" and exact_txt_splitted[
                1].isdigit():
            ch = int(exact_txt_splitted[1])
            ch_o = client.get_channel(ch)
            await ch_o.send(" ".join(exact_txt_spl[2:]))

        if exact_txt_splitted[0] == "send_au" and exact_txt_splitted[
                1].isdigit():
