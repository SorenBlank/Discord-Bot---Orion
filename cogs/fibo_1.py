        ...
        try:     
            if message.channel.id == id:
                if r != 0 and x.isdigit():
                    digit = r
                    past_digit = raw["past"]
                    last_digit = info["last"]
                    last_author = info["author"]
                    ...
                    if past_digit == 0 and last_digit == 0:
                        ...
                        fc_cur.update_one({"channel":message.channel.id},{"$set":{"past":last_digit,"last":equal,"author":message.author.id}})
                        await message.add_reaction("✅")

                    elif last_author == message.author.id:
                        await message.add_reaction("❎")
                        fc_cur.update_one({"channel":message.channel.id},{"$set":{"past":0,"last":0,"author":0}})
